...
int main()
{
    ...
    unsigned char *pBuffer = (unsigned char *)malloc(65536); //Its Big!
    ...
    while(1)
    {
        ...
        //Receive a packet
        data_size = recvfrom(sock_raw , pBuffer , 65536 , 0 , &saddr , &saddr_size);
        if(data_size <0 )
        {
...
void ProcessPacket(unsigned char* pBuffer, int size)
{
	std::cout << "*****************************" << std::endl;
    ...
    switch (iph->protocol) //Check the Protocol and do accordingly...
    {
        ...
        case 6:  //TCP Protocol
            ++tcp;
            print_tcp_packet(pBuffer , size);
            break;

        case 17: //UDP Protocol
            ++udp;
            print_udp_packet(pBuffer , size);
            break;
...
void print_ip_header(unsigned char* Buffer, int Size)
{
    ...
    std::cout << "src: " << inet_ntoa(source.sin_addr) << " , dest: " << inet_ntoa(dest.sin_addr) << std::endl;
}
...
void print_tcp_packet(unsigned char* Buffer, int Size)
{
    ...
    unsigned char* pData = Buffer + iphdrlen + tcph->doff;
